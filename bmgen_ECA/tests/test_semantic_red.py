...
def test_snake_case():
    ...
    assert snake_case("DoorECU-BodyCan0") == "door_ecu__body_can0"
...
def test_v2_validate_green(schema_v2_path):
    ...
    assert [r.rule_id for r in ir.rx_frames[("DoorECU-BodyCan0", "DoorCmd")]] == [
        "receive_target"
    ]
...
def test_red_delete_rx(schema_v2_path, tmp_path):
    ...
    line = '      - signal: "[DoorECU-BodyCan0]DoorCmd.TargetPosition"\n'
    assert line in text
    out = tmp_path / "x.yaml"

...
class CentralHPC:
    ...
    _camera_input_latched: bool = False
    ...
    async def CAD_logic(self, frame: Frame) -> None:
        ...
        if "CameraInput.ChildDetectedByCamera" in frame.signals:
            self._camera_input_latched = bool(frame.signals["CameraInput.ChildDetectedByCamera"])
        ...
        _weighted_sum = (1.0 * (1 if self._seat_input_latched else 0)) + (1.0 * (1 if self._camera_input_latched else 0)) + (2.0 * (1 if self._airbag_status_report_latched else 0))
